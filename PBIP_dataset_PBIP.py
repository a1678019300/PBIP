...
train_csv_dir = 'PBIP_train_set.csv'
test_csv_dir = 'PBIP_test_set.csv'
phage_unirep_dir = "data/PBIP/phage"
host_unirep_dir = "data/PBIP/host"
fold_result_dir = '10_fold_result/dataset_PBIP'
test_result_dir = 'test_result/dataset_PBIP'
model_result_dir = 'model/dataset_PBIP'
...
def main():
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(train_phage)):
        ...
        X_p_train_resampled, _ = smote.fit_resample(X_p_train, y_train)
        X_b_train_resampled, y_train_resampled = smote.fit_resample(X_b_train, y_train)
    ...
    smote_final = SMOTE(sampling_strategy='minority', random_state=RANDOM_SEED)
    train_phage_resampled, _ = smote_final.fit_resample(train_phage_scaled, train_labels)
    train_host_resampled, train_labels_resampled = smote_final.fit_resample(
        train_host_scaled, train_labels
    )
